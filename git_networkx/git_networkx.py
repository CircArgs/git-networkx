...
import git_networkx.git_graph_class as gg
...
def GitNX(git_path, nodes=ALL_NODES):
    G=nx.DiGraph()
    git_graph = gg.GitGraph(git_path).build_graph()
    ...
    if "t" in nodes:
        for _t in git_graph.trees:
            ...
            for e in git_graph.trees[_t]:
                if e[0] in node_set:
                    G.add_edge(t, resolve_child_type(e, t, git_graph)(e[0]))
    if "c" in nodes:
        for _c in git_graph.commits:
            ...
            for _e in git_graph.commits[_c]:
                if _e in node_set:
                    G.add_edge(c, resolve_child_type(_e, c, git_graph)(_e))
    if "l" in nodes:
        for _l in git_graph.local_branches:
            ...
            if e in node_set:
                G.add_edge(l, resolve_child_type(e, l, git_graph)(e))
    if "h" in nodes:
        ...
        if e in node_set:
            G.add_edge(h, resolve_child_type(e, h, git_graph)(e))
    if "r" in nodes:
        for _r in git_graph.remote_branches:
            ...
            if e in node_set:
                G.add_edge(r, resolve_child_type(e, r, git_graph)(e))
    if "d" in nodes:
        for _d in git_graph.remote_heads:
            ...
            if e in node_set:
                G.add_edge(d, resolve_child_type(e, d, git_graph)(e))
    if "s" in nodes:
        for _s in git_graph.remote_servers:
            ...
            for _e in git_graph.remote_servers[_s]:
                if _e in node_set:
                    G.add_edge(s, resolve_child_type(_e, s, git_graph)(_e))
    if "a" in nodes:
        for _a in git_graph.annotated_tags:
            ...
            if e in node_set:
                G.add_edge(a, resolve_child_type(e, a, git_graph)(e))
    if "g" in nodes:
        for _g in git_graph.tags:
            ...
            e = git_graph.tags[_g]
            if e in node_set:
                G.add_edge(g, resolve_child_type(e, g, git_graph)(e))
    # if "u" in nodes:  # no color (only edges)
    #     for u in git_graph.upstreams:
    #         e = git_graph.upstreams[u]
    #         if e in node_set:
    #             G.add_edge(u, resolve_child_type(e, u, git_graph)(e))
    return G